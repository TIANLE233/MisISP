...
from .data_util import paired_paths_from_folder_avif, paired_paths_from_lmdb, \
    paired_paths_from_meta_info_file


@DATASET_REGISTRY.register()
class RTSRDataset(data.Dataset):  # 暂停bit0的使用
    def __init__(self, opt) -> None:
        super(RTSRDataset, self).__init__()
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths = paired_paths_from_folder_avif([self.lq_folder, self.gt_folder], ['lq', 'gt'], self.filename_tmpl)
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.bit == 0:
            ...
            gt_path = self.paths[index]['gt_path']
            img_gt = np.asarray(Image.open(gt_path))
            img_gt = img_gt.astype(np.float32)

            lq_path = self.paths[index]['lq_path']
            img_lq = np.asarray(Image.open(lq_path))

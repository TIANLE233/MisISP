...
if __name__ == '__main__':
    ...
    # 假设 model 是已经定义好的模型实例
    # 例如: model = StyleModel(upscale=2, num_in_ch=3, num_out_ch=3, task='some_task')
    ...
    class ModelWrapper(nn.Module):
        ...
        def forward(self, x, gt):
            return self.model(x, gt)

    # 包装原始模型
    wrapped_model = ModelWrapper(model)

    # 生成一个随机的输入图像张量和目标张量，确保 requires_grad=True
    input_image = torch.rand(2, 3, 64, 64, requires_grad=True, dtype=torch.double)  # double precision for gradcheck
    target_image = torch.rand(2, 3, 64, 64, dtype=torch.double)

    # 进行梯度检查
    input_tuple = (input_image, target_image)  # gradcheck expects a tuple of inputs
    test = gradcheck(wrapped_model, input_tuple, eps=1e-6, atol=1e-4)

    if test:
        print("Model supports gradient backpropagation correctly.")
    else:
        print("Model has issues with gradient backpropagation.")

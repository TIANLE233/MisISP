...
from basicsr.utils import parse_options, make_exp_dirs, mkdir_and_rename
        ...
        if phase == 'train':
            ...
            if extra_datasets is not None:
                for _, extra_dataset in extra_datasets.items():
                    ...
                    if extra_dataset.get('dataroot_masko') and extra_dataset.get('dataroot_maskp'):
                        _dataset_opt['dataroot_masko'] = extra_dataset['dataroot_masko']
                        _dataset_opt['dataroot_maskp'] = extra_dataset['dataroot_maskp']
...
def train_pipeline(root_path):
    ...
    shutil.copyfile(args.path_arch, osp.join(opt['path']['experiments_root'], osp.basename(args.path_arch)))
    shutil.copyfile(args.path_model, osp.join(opt['path']['experiments_root'], osp.basename(args.path_model)))
    shutil.copyfile(args.path_data, osp.join(opt['path']['experiments_root'], osp.basename(args.path_data)))

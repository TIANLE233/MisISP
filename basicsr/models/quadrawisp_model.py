...
class QuadRAWISPModel(BaseModel):
    def __init__(self, opt) -> None:
        super(QuadRAWISPModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        if 'mask' in data:  # Add mask handling
            self.mask = data['mask'].to(self.device)
        if 'ref' in data:  # Add mask handling
    ...
    def optimize_parameters(self, current_iter):
        ...
        # pixel loss
        if self.cri_pix:
            l_pix = self.cri_pix(self.output * self.mask, self.gt * self.mask)
            l_total += l_pix
            loss_dict['l_pix'] = l_pix

        if self.cri_pac:
            l_pac = self.cri_pac(self.output, self.ref)
            l_total += l_pac
        ...
        if self.cri_psnr:
            l_psnr = self.cri_psnr(self.output* self.mask, self.gt* self.mask)
            l_total += l_psnr

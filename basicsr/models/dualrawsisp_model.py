...
class RAWSISPModel(BaseModel):
    def __init__(self, opt) -> None:
        super(RAWSISPModel, self).__init__(opt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_pac:
            l_pac = self.cri_pac(self.output, self.gt)
            l_total += l_pac
        ...
        if self.cri_psnr:
            l_psnr = self.cri_psnr(self.output, self.gt)
            l_total += l_psnr
